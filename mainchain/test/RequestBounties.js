...
contract('RequestBounties', function(accounts) {
  ...
  it('should send added bounty value to funds contract', core.redeploy(accounts, async (underTest, mockFundable) => {
    await core.addRequest(underTest, accounts);
    await core.addToBounty(underTest, accounts, undefined, undefined, 50000, 50000);
    assert.equal(await web3.eth.getBalance(mockFundable.address), core.BOUNTY + 50000);
  }));

  it('should fire a BountyAdded event when a bounty is added', core.redeploy(accounts, async (underTest) => {
    await core.addRequest(underTest, accounts);
    await core.addToBounty(underTest, accounts, undefined, undefined, 50000, 50000);
    let bountyAdded = underTest.getPastEvents('BountyAdded',{fromBlock: 0, toBlock: 'latest'});
    let logs = await getEvents(bountyAdded);
    assert.equal(web3.utils.hexToUtf8(logs[logs.length-1].args.requestId), web3.utils.hexToUtf8(core.ID), 'Id incorrect');
    assert.equal(logs[logs.length-1].args.addedAmount, 50000, 'Added amount incorrect');
    assert.equal(logs[logs.length-1].args.newTotal, core.BOUNTY + 50000, 'New total incorrect');
  }));
