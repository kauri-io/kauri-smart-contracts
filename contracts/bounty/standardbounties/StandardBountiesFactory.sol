...
contract StandardBountiesFactory {
  ...
  function createBounty(address _issuer, address _arbiter, string _data, uint _deadline)
  public
  returns (address)
  {
    address newBounty = new Proxy(masterCopy);
    bounties.push(StandardBounty(newBounty));
    bounties[bounties.length - 1].initializeBounty(_issuer, _arbiter, _data, _deadline);
